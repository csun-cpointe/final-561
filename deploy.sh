...
startup() {
  ...
  helm upgrade --install $INFRA_NAME final-561-infrastructure \
     --values final-561-infrastructure/values.yaml \
     --values final-561-infrastructure/values-dev.yaml
  if ! kubectl rollout status --namespace argocd deployment/argocd-server --timeout=30s; then
    exit $?
  fi
  argocd repo add https://github.com/csun-cpointe/final-561.git --server localhost:30080 --plaintext --insecure-skip-server-verification
}
...
deploy() {
  ...
  if is_app_running; then
    echo "final-561 is deployed"
  else
    branch=$(git rev-parse --abbrev-ref HEAD)
    echo "Deploying final-561 from branch '$branch'..."
    argocd app create $APP_NAME \
      --server localhost:30080 --plaintext \
      --dest-namespace final-561 \
      --dest-server https://kubernetes.default.svc \
      --repo https://github.com/csun-cpointe/final-561.git \
      --path final-561-deploy/src/main/resources \
      --revision $branch \
...
down() {
  if is_app_running; then
    ...
    echo "final-561 is not deployed"
  fi
}
